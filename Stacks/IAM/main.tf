...
resource "oci_identity_policy" "example_policy" {
  ...
  compartment_id = var.tenancy_ocid
  statements = [
    "Allow group ${oci_identity_group.example_group.name} to read instances in compartment ${compute}",
    "Allow group ${oci_identity_group.example_group.name} to inspect instances in compartment ${applications}",
  ]
}
